...
var sAbsoluteLocation =  location.protocol+"//"+location.hostname+sLPath.substring(0,sLPath.indexOf("/",vB)+1); // <href>
...
var sSelfName = sLPath.substring(sLPath.lastIndexOf("/")+1,sLPath.length); // <href>
var sJumpLocation = "";
...
var lLoop_SlideShow = true; // false; //
...
function Entry_SlideShow_Image( nInterval, lLoop, sImageObjName, sImagePath, d1sList_ImageName, vImageWidth, vImageHeight, sLink ){
   ...
   document.write(" onmouseover='Set_SlideShow_Image(\""+nInterval+"\",\""+lLoop+"\",1,\""+sImageObjName+"\");' ");
...
function Set_SlideShow_Image( nInterval, lLoop, nSign, sImageObjName ){
   ...
   if(
    ...
    ( sImageObjName!=sObjName )
     ...
     window.status = "Slide："+(nNext_SlideShow_Image+1)+"/"+(d1List_SlideShow_ImageName.length)+"; ";
...
function Switch_SlideShow_Image( nSign ){
   ...
   if(
    ( nNext_SlideShow_Image<0 )||
    ( d1List_SlideShow_ImageName.length<=nNext_SlideShow_Image )
   ){
...
var lJavaScript_LoadCheck_common0 = true; // false; //
var lJavaScriptLoading_Common = true; // false; //
